...
setwd("~/Documents/Dissertation/MAX_ENT_CODE/burden")
...
setwd("~/Documents/Dissertation_KS/MAX_ENT_CODE/burden")
# Note that you can pass in the loaded DDI into the `read_ipums_micro()`
cps_ddi <- read_ipums_ddi("usa_00023.xml")
cps_data <- read_ipums_micro(cps_ddi, data_file = "usa_00023.dat.gz", verbose = FALSE)
...
util_pctile <- cps_usa %>% 
  group_by(YEAR) %>%
  do(util_pctile = reldist::wtd.quantile(.$util_burden, q = 0.98, weight = .$HHWT)) %>% 
...
for (i in 2008:2019){
  histogram<- cps_usa %>% 
    filter(year==i) %>% 
    hist(as.numeric(util_burden))
    ggsave(histogram, file = paste(i,"util_burden.pdf"), path="~/Documents/Dissertation/MAX_ENT_CODE/burden/histograms")
}
cps_usa %>% 
  filter(YEAR==2019) %>% 
  ggplot( aes(x=util_burden, fill=SEX)) +
...
test_19<-cps_usa %>% 
  filter(YEAR==2019,
         util_burden<=1)
#We need to investigate the households that are not reporting income, but still owe utility bills. 
#For this reason, the analysis separates utility burdens into two different groups. Observations with a value 
# greater than 1 (UB > 1) is 805095 and observations less than or equal to 1 totals 11912288 from 2008-2019.
# AT the moment let's look at thos below and then figure ou
library(questionr)
library(Hmisc)
for (i in 2008:2019){
  above<- above_df %>% 
...
set.seed(0308)
  ...
  dfa<-data.frame(
  sex=above$SEX,
  burden=(above$util_burden))

dfb<-data.frame(
  sex=below$SEX,
  burden=(below$util_burden))
}
...
head(df)
# Change colors
p<-ggplot(df, aes(x=burden)) + 
  geom_histogram(color="black", fill="white")
p
# Overlaid histograms
ggplot(df, aes(x=burden, color=sex)) +
...
#Look at Noe's original code to see what it looks like - 8/14

set.seed(0308)
wtc_xyz_histogram <- cps_usa %>%
